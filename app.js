...
app.post("/api/getMeetingLink", async (req, res) => {
  ...
  if (await bcrypt.compare(passwo, req.session.person.password)) {
    const slot = await finding(
      client,
      { username: req.session.person.username },
      "slots"
    ); //find the user with userid as given userid
    //links for doctors
    //meet.google.com/hwx-fmjp-iou - subash
    // meet.google.com/nkn-bznq-zqk-shalini
    // meet.google.com/mke-vwwk-ivp - amit
    if (slot) {
      // Update the meet link in the below line according to the doctor name from the input
      if (slot.doctor == "Subash") {
        res.json({
          success: true,
          meetLink: "https://meet.google.com/hwx-fmjp-iou",
        });
      } else if (slot.doctor == "Shalini") {
        res.json({
          success: true,
          meetLink: "https://meet.google.com/nkn-bznq-zqk",
        });
      } else if (slot.doctor == "Amit") {
        res.json({
          success: true,
          meetLink: "https://meet.google.com/mke-vwwk-ivp",
        });
      }
    } else {
      res
...
app.delete("/deleteAlert/:id", async (req, res) => {
  try {
    ...
    await client
      .db("Medi")
      .collection("ale")
      .deleteOne({ _id: new ObjectId(alertId) }); // Delete alert by ID
...
app.post("/updatedonor", async (req, res) => {
  ...
  try {
    ...
    const existingDonor = await client
      .db("Medi")
      .collection("user")
      .findOne({ email });
    ...
    if (existingDonor) {
      ...
      const previousDonDate = existingDonor.last_don
        ? new Date(existingDonor.last_don)
        : null;
      ...
      if (previousDonDate) {
        ...
        if (diffDays < 90) {
          return res
            .status(400)
            .send("The last donated date must be at least 90 days apart.");
        }
      }
...
app.post("/updatemedicine", async (req, res) => {
  ...
  const client = new MongoClient(uri);
  try {
    const existingMedicine = await client
      .db("Medi")
      .collection("meds")
      .findOne({ slno });
    ...
    if (existingMedicine) {
      await client
        .db("Medi")
        .collection("meds")
        .updateOne({ slno }, { $set: { name, type, use: useArray, stock } });
...
app.post("/signup", async (req, res) => {
  try {
    ...
    await inserting(
      client,
      { username, email, pass: hashedPassword, first_name, last_name, login },
      "user"
    );
...
app.post("/set-reminder", async (req, res) => {
  ...
  try {
    ...
    await inserting(
      client,
      { medName, medTimes, frequency, email },
      "medications"
    );
...
cron.schedule("*/1 * * * *", async () => {
  ...
  try {
    ...
    // Fetch medications due at the current time
    const medications = await finding(
      client,
      { medTimes: { $in: [now] } },
      "medications"
    );
